...
**Copy and paste this entire script block into your terminal on the target node.**

The script will:
1. Download and extract the deployment bundle
2. Install pi and Ollama
3. Run hardware detection
4. Run model benchmark
5. Display instructions for next steps

**Prerequisites:**
- Phone tether connected (USB tethering enabled)
...
BUNDLE_URL="https://gist.github.com/<GIST_ID>/raw/network-troubleshooting-bundle.tar.gz"
curl -L "$BUNDLE_URL" -o /tmp/nt-bundle.tar.gz
cd ~ && tar -xzf /tmp/nt-bundle.tar.gz
echo "✓ Bundle extracted to ~/network-troubleshooting-bundle"
echo ""
